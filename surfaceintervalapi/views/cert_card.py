...
from surfaceintervalapi.models import Diver, Certification_Card
...
class CertCardView(ViewSet):
    ...
    def retrieve(self, request, pk=None):
        try:
            cert = Certification_Card.objects.get(pk=pk, diver__user=request.auth.user)
    ...
    def list(self, request):
        cert_cards = Certification_Card.objects.filter(diver__user=request.auth.user)
        serializer = CertCardSerializer(
            cert_cards, many=True, context={'request': request})
        return Response(serializer.data)
    ...
    def create(self, request):
        ...
        try:
            cert = Certification_Card.objects.create(
                diver=diver,
            ...
            serializer = CertCardSerializer(
                cert, many=False, context={"request": request})
    ...
    def partial_update(self, request, pk):
        ...
        try:
            cert = Certification_Card.objects.get(diver=diver, pk=pk)
    ...
    def destroy(self, request, pk=None):
        ...
        try:
            cert = Certification_Card.objects.get(pk=pk, diver=diver)
            cert.delete()

            return Response({"Cert deleted"}, status=status.HTTP_204_NO_CONTENT)

        except Certification_Card.DoesNotExist as ex:
