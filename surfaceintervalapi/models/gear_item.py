...
class Gear_Item(models.Model):
    ...
    def dives_since_last_service(self):
        ...
        if self.last_serviced is None and self.purchase_date:
            dives = Dive.objects.only('gear_set').filter(diver=self.diver, date__gte=self.purchase_date)
        elif self.last_serviced:
            dives = Dive.objects.only('gear_set').filter(diver=self.diver, date__gte=self.last_serviced)
        else:
            pass
